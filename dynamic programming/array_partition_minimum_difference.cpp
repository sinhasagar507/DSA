...
int f(vector<int>& v, vector<int>& res, int tot_sum, int i, int n)
{
	if(i == 0)
	{
		...
		else 
		{
			...
			return (res_sum != tot_sum) ? abs((tot_sum-res_sum)-res_sum): INT_MAX; 
		}
	}
	
	// Don't take the element 
	int lsum = f(v, res, tot_sum, i-1, n); 
	
	res.push_back(v[i]); 
	int rsum = f(v, res, tot_sum, i-1, n);
	res.pop_back(); 
